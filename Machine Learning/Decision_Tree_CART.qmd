...
# CART
library(rpart) # creates CART Trees
library(rules)
library(hardhat)
...
recipe <- recipe(EVT ~ ., data = train_data) %>% 
  step_smotenc(
    EVT,                     
    over_ratio = tune(),       # Oversampling SMOTE
    neighbors  = 5,
    skip       = TRUE
  ) %>% 
  step_downsample(             # Downsampling
    EVT,
    under_ratio = tune(),
    skip = TRUE
  )
...
#  cost_complexity = 0,
  ...
  set_engine("rpart",
             model = TRUE) # for more details of the plot in rpart.plot
...
cart_parameter_set <- parameters(
#  cost_complexity(range = c(1e-8, 1e-4),
  tree_depth(range = c(1L, 6L)),
  min_n     (range = c(10L, 40L)),
  over_ratio(range = c(0.0, 0.1)),
  under_ratio(range = c(1.0, 1.5))
)

# Create a grid for Race ANOVA
cart_parameter_grid_race <- grid_space_filling(cart_parameter_set,
                                                  size = 100)
...
cart_parameter_grid_search <- grid_regular(cart_parameter_set,
...
# cart_tune_results <- tune_race_anova(
#   cart_workflow,
#   resamples = cv_folds,
#   grid      = cart_parameter_grid_race,
...
cart_tune_results <- tune_grid(
    cart_workflow,
    resamples = cv_folds,
    grid      = cart_parameter_grid_search,
    metrics   = metrics_tune_grid)
...
Here: 5 (Oversampling Ratios) \* 2 Distance Metrics \* 3 Weight functions \* 10 Neigbours \* 3 Metrics = 1200

```{r}

# cart_tune_results$.metrics
...
best_cart <- select_best(cart_tune_results,
                       metric = "roc_auc")
...
all_fold_metrics <- collect_metrics(cart_tune_results, summarize = FALSE)
...
if ("cost_complexity" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "cost_complexity",
...
if ("tree_depth" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "tree_depth",
                        label = "Tree Depth"))
}
...
**Undersampling Ratio (under_ratio)**
...
plot_one_metric <- function(metric_name) {
  ggplot(
    heat_df %>% filter(metric_label == metric_name),
    aes(over_ratio, under_ratio, fill = mean_val)
  ) +
    geom_tile() +
    geom_text(aes(label = sprintf("%.1f", mean_val)), colour = "white", size = 3) +
    scale_fill_gradient(
      low    = "white",
...
cart_metrics <- tune::collect_metrics(cart_tune_results)

cart_hyperparams <- cart_metrics %>% 
  select(-.metric, -.estimator, -mean, -n, -std_err, -.config)

# Plot all available Hyperparameters
print(colnames(cart_hyperparams)) 
...
hyperparameter_1 <- "under_ratio"   
hyperparameter_2 <- "tree_depth"   
hyperparameter_3 <- "min_n"    
...
pr_auc_subset_3_params <- cart_metrics %>%
...
cart_hyperparameter_workflow <- finalize_workflow(cart_workflow, best_cart)
cart_hyperparameter_workflow
...
final_cart_fit <- fit(cart_hyperparameter_workflow, data = train_data)
final_cart_fit
...
# final_cart_fit <- fit(cart_workflow, data = train_data)
# 
# final_cart_fit
...
train_probs <- predict(final_cart_fit, new_data = train_data, type = "prob") %>%
...
metrics_grid <- map_dfr(thresholds, function(thr) {
  truth <- factor(train_probs$EVT, levels = c("0","1"))
  ...
  tibble(
    ...
    # falls "0" positiv ist, hier auf "first" ändern
  )
})
...
cart_predictions_train <- predict(final_cart_fit,
                                 new_data = train_data,
                                 type = "prob") %>%
  mutate(
...
# 1. Predicted probabilities + true labels (Test)
test_probs <- predict(final_cart_fit, new_data = test_data, type = "prob") %>%
  bind_cols(test_data %>% select(EVT)) %>%
  mutate(EVT = factor(EVT, levels = c("0","1")))  # "1" sollte positiv sein, prüfen!
...
metrics_grid <- map_dfr(thresholds, function(thr) {
  ...
  tibble(
    threshold   = thr,
    # wenn "1" wirklich positiv ist:
    sensitivity = sensitivity_vec(truth, preds, event_level = "second"),
    specificity = specificity_vec(truth, preds, event_level = "second")
    # wenn "0" positiv ist, ändere beide zu event_level = "first"
  )
})
...
# Define a custom threshold
custom_threshold <- 0.6  # The higher the threshold, the more likely the model will predict a positive class ("1")

# Make predictions with probabilities for the test data
cart_predictions_test <- predict(final_cart_fit,
                                new_data = test_data,
                                type = "prob") %>%
  mutate(
    pred_class = if_else(.pred_1 >= custom_threshold, "1", "0"), # Apply custom threshold
...
cm_cart_train <- confusionMatrix(data = cart_predictions_train$pred_class,
                reference = cart_predictions_train$EVT,
                positive = "1")
...
cm_cart_test <- confusionMatrix(data = cart_predictions_test$pred_class,
                reference = cart_predictions_test$EVT,
                positive = "1")
...
boot_results <- boot::boot(
  data      = cart_predictions_test,      # dataset to bootstrapp      
...
# Training
probabilities_ml_train <- cart_predictions_train$.pred_1

# Testing
probabilities_ml_test <- cart_predictions_test$.pred_1
...
conf_matrix_counts <- cart_predictions_test %>%
  mutate(
    type = case_when(
...
This chunk executes a **5‑fold *outer* cross‑validation** on a *fixed* CART specification. We skip the inner tuning loop because:
...
Hyperparameters from section above: `select_best(cart_tune_results, metric = "roc_auc")`

```{r}

# Model Hyperparameters

cart_spec_fixed <- decision_tree(
  mode            = "classification",
#  cost_complexity = 1,
  tree_depth      = 2,
  min_n           = 40L
...
evaluate_fold_cart <- function(split,
  ...
  fold_recipe <- recipe(
    ...
    step_mutate_at(all_nominal_predictors(),
                   fn = ~ factor(.x, ordered = is.ordered(.x))) %>%
    step_mutate(
      vigilance_4ISS           = factor(vigilance_4ISS,
                                        levels = 0:2,
                                        ordered = FALSE),
  ...
  fit_outer <- workflow() %>% 
    ...
    add_model(cart_spec_fixed) %>% 
    fit(train_outer)

  # Predict 
  probs <- predict(fit_outer, test_outer, type = "prob")$.pred_1
  preds_class <- factor(ifelse(probs >= threshold, "1", "0"),
                        levels = c("1","0"))
  ...
  # Dataframe for calculating PRAUC
  pr_df <- tibble(truth = test_outer$EVT, prob = probs) %>% 
...
outer_results_cart <- outer_folds %>%
  mutate(
    results = map(
      splits,
      evaluate_fold_cart,
...
metrics_df_folds <- outer_results_cart %>% 
  transmute(
...
**The PR-AUC is so high** because a CART stump with a single rule produces only two distinct predicted probabilities—one per leaf—which substantially inflates the PR-AUC.**\
**
...
cm_list <- map(outer_results_cart$results, "cm")
...
cart_outer_preds  <- map_dfr(outer_results_cart$results,
...
# Random-Forest ROC
roc_cart <- pROC::roc(cart_outer_preds$truth, cart_outer_preds$score,
...
# Extract the underlying rpart object from the fitted workflow
cart_rpart <- final_cart_fit %>%
  extract_fit_parsnip() %>%  # access the fitted model object
  .$fit                      # retrieve the rpart tree component

# Plot the tree using rpart.plot
rpart.plot(
  cart_rpart,
  type   = 2,    # draw nodes as boxes
  extra  = 104,  # display node sample counts and leaf probabilities
  under  = TRUE, # show probabilities beneath the node boxes
  faclen = 0,    # print full factor levels without abbreviating
...
cart_model <- extract_fit_engine(final_cart_fit)
...
importance_values <- cart_model$variable.importance
...
saveRDS(cart_hyperparameter_workflow, "models/final_cart_workflow.rds")

# Save the final model
saveRDS(final_cart_fit, "models/final_cart_model.rds")
