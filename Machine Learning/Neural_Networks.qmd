...
abstract: "This document shows the process of training Neural Networks for preclinical stroke triage. It covers data preprocessing steps, including handling missing values via bagged tree imputation and scaling, followed by hyperparameter tuning using Racing Methods and, alternatively, grid search. The document compares the performance of the tuned Neural Network against a fixed hyperparameter model and the 4-Item Stroke Scale (4I-SS) on a held-out test set and outer cross-validation. Evaluation metrics, including ROC and PR curves, along with confusion matrices and confidence intervals obtained through bootstrapping, are presented to assess model efficacy"
editor: visual
---
...
recipe <- recipe(EVT ~ ., data = train_data) %>%
  # 1) Log-transform skewed numerics (e.g. TOO)
  step_log(all_numeric_predictors(), base = 10, offset = 1) %>%
  
  # 2) Min–max scaling into [0,1]
  step_range(all_numeric_predictors(), min = 0, max = 1) %>%
  
  # 3) SMOTE-ENC: upsample minority to 50% of majority
  step_smotenc(EVT, over_ratio = tune(), neighbors = 5, skip = TRUE) %>%
  
  # 4) Downsample majority to match minority (1:1)
  step_downsample(EVT, under_ratio = tune(), skip = TRUE) %>%
  
  # 5) One-hot encode all factor predictors
...
nn_parameter_set <- parameters(
  hidden_units(range = c(7L, 10L)),        
  epochs(range       = c(275L, 350L)),     
  penalty     (range  = c(1e-6, 1e-1)),
  over_ratio(range   = c(0.1, 0.5)),      
  under_ratio(range  = c(1.0, 3.0))       
)


# Create a grid for Race ANOVA
nn_parameter_grid_race <- grid_space_filling(nn_parameter_set,
                                                  size = 50)
...
nn_parameter_grid_search <- grid_regular(nn_parameter_set,
...
nn_tune_results <- tune_race_anova(
  nn_workflow,
  resamples = cv_folds,                
  grid      = nn_parameter_grid_race,                
...
best_nn <- select_best(nn_tune_results,
                       metric = "roc_auc")
...
all_fold_metrics <- collect_metrics(nn_tune_results, summarize = FALSE)
...
if ("hidden_units" %in% names(prepared_data)) {
...
if ("epochs" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "epochs",
...
if ("penalty" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "penalty",
                        label = "Weight-Decay (penalty)"))
}

```

**Oversampling Ratio (over_ratio)**

```{r}

if ("over_ratio" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "over_ratio",
                        label = "Oversampling Ratio (over_ratio)"))
}
...
heat_df <- nn_tune_results %>% 
  ...
  mutate(metric_label = dplyr::recode(.metric,
                               pr_auc      = "AUCPR",
                               roc_auc     = "AUCROC",
                               sensitivity = "Sensitivity",
                               specificity = "Specificity"))
...
nn_metrics <- tune::collect_metrics(nn_tune_results)

nn_hyperparams <- nn_metrics %>% 
  select(-.metric, -.estimator, -mean, -n, -std_err, -.config)

# Plot all available Hyperparameters
print(colnames(nn_hyperparams)) 
...
hyperparameter_1 <- "hidden_units"     
hyperparameter_2 <- "epochs"  
hyperparameter_3 <- "penalty"  
...
pr_auc_subset_3_params <- nn_metrics %>%
...
nn_hyperparameter_workflow <- finalize_workflow(nn_workflow, best_nn)
nn_hyperparameter_workflow
...
final_nn_fit <- fit(nn_hyperparameter_workflow, data = train_data)
final_nn_fit
...
# final_nn_fit <- fit(nn_workflow, data = train_data)
# 
# final_nn_fit
...
# 1. Get predicted probabilities + true labels (train set)
train_probs <- predict(final_nn_fit, new_data = train_data, type = "prob") %>%
  bind_cols(train_data %>% select(EVT))

# 2. Define grid of thresholds
thresholds <- seq(0, 1, by = 0.01)

# 3. Compute sensitivity & specificity per threshold
metrics_grid <- map_dfr(thresholds, function(thr) {
  truth <- factor(train_probs$EVT, levels = c("0","1"))  # Level-Order fix
  preds <- factor(if_else(train_probs$.pred_1 >= thr, "1", "0"),
                  levels = c("0","1"))
...
nn_predictions_train <- predict(final_nn_fit,
                                 new_data = train_data,
                                 type = "prob") %>%
  mutate(
...
# 1. Get predicted probabilities + true labels
test_probs <- predict(final_nn_fit, new_data = test_data, type = "prob") %>%
  bind_cols(test_data %>% select(EVT)) %>%
  mutate(EVT = factor(EVT, levels = c("0","1")))  # "1" = positiv

# 2. Define a fine grid of thresholds
thresholds <- seq(0, 1, by = 0.01)

# 3. For each threshold, compute sens & spec
metrics_grid <- map_dfr(thresholds, function(thr) {
  truth <- factor(test_probs$EVT, levels = c("0","1"))
  preds <- factor(if_else(test_probs$.pred_1 >= thr, "1", "0"),
                  levels = c("0","1"))
  tibble(
    threshold   = thr,
    sensitivity = sensitivity_vec(truth, preds, event_level = "second"),
...
Predict classes for the test data using a **custom threshold** to assess pennormance on data that the model has never seen before.
...
nn_predictions_test <- predict(final_nn_fit,
                                new_data = test_data,
                                type = "prob") %>%
  mutate(
    pred_class = if_else(.pred_1 >= custom_threshold, "1", "0"), # Apply custom threshold
...
cm_nn_train <- confusionMatrix(data = nn_predictions_train$pred_class,
                reference = nn_predictions_train$EVT,
                positive = "1")
...
cm_nn_test <- confusionMatrix(data = nn_predictions_test$pred_class,
                reference = nn_predictions_test$EVT,
                positive = "1")
...
boot_results <- boot::boot(
  data      = nn_predictions_test,        # dataset to bootstrapp      
...
# Training
probabilities_ml_train <- nn_predictions_train$.pred_1

# Testing
probabilities_ml_test <- nn_predictions_test$.pred_1
...
conf_matrix_counts <- nn_predictions_test %>%
  mutate(
    type = case_when(
...
This chunk executes a **5‑fold *outer* cross‑validation** on a fixed Neural Network specification. We skip the inner tuning loop because:
...
Hyperparameters from section above: `select_best(nn_tune_results, metric = "roc_auc")`
...
custom_over_ratio  <- 0.1408163
custom_under_ratio <- 1.816327
...
evaluate_fold_nn <- function(split,
                             threshold   = custom_threshold,
                             over_ratio  = custom_over_ratio,
                             under_ratio = custom_under_ratio,
                             seed        = 100) {
  set.seed(seed)
  ...
  # Recipe
  fold_recipe <- recipe(
      EVT ~ gender + age + bp_systolic + pulse + blood_glucose + temperature +
            vigilance_4ISS + gaze_head_deviation_4ISS + hemiparesis_4ISS +
            aphasia_dysarthria_4ISS + medication_anticoags + TOO +
            exclusion_TOO + pupil_reaction_abnormal,
      data = train_outer
    ) %>% 
    step_log(all_numeric_predictors(), base = 10, offset = 1) %>% 
    step_range(all_numeric_predictors(), min = 0, max = 1) %>% 
    step_smotenc(EVT, over_ratio = over_ratio, neighbors = 5, skip = TRUE, seed = seed) %>% 
    step_downsample(EVT, under_ratio = under_ratio, skip = TRUE, seed = seed) %>% 
    step_dummy(all_nominal_predictors(), one_hot = TRUE)
  
  # Fit 
  fit_outer <- workflow() %>% 
    add_recipe(fold_recipe) %>% 
    add_model(nn_spec_fixed) %>% 
    fit(train_outer)
  
  # Predict 
  probs       <- predict(fit_outer, new_data = test_outer, type = "prob")$.pred_1
  preds_class <- factor(ifelse(probs >= threshold, "1", "0"), levels = c("1","0"))

  # Metrics
  cm     <- caret::confusionMatrix(preds_class, test_outer$EVT, positive = "1")
  pr_df  <- tibble(truth = test_outer$EVT, prob = probs) %>%
              mutate(truth = forcats::fct_relevel(truth, "1", "0"))
  aucroc <- pROC::auc(pROC::roc(test_outer$EVT, probs, quiet = TRUE))
  aucpr  <- yardstick::pr_auc(pr_df, truth, prob)$.estimate
  ...
  # Brier
  truth_num <- as.numeric(as.character(test_outer$EVT))
...
metrics_df_folds <- outer_results_nn %>% 
  transmute(
...
cm_list <- map(outer_results_nn$results, "cm")
...
evaluate_4iss <- function(split) {
  ...
  preds_4iss <- test_outer %>%
    mutate(
      classification = factor(if_else(sum_4ISS > 2, "1", "0"), levels = levels(EVT)),
      actual = EVT
    )
...
Possible approaches include: repeated nested CV (e.g., 10×5 folds) to obtain more outer estimates for bootstrapping (Data Science Stack Exchange); full data bootstrapping with nested CV per sample to capture data and model uncertainty (Cai et al., arXiv); analytical or random-effects variance modeling for CI estimation (Cai et al., Bates et al., arXiv); and simple t-based intervals over fold means, though often overly optimistic.Create a table
...
nn_outer_preds  <- map_dfr(outer_results_nn$results,
...
# Random-Forest ROC
roc_nn <- pROC::roc(nn_outer_preds$truth, nn_outer_preds$score,
...
Extract the model from tidymodels workflow

```{r}

tidymodels_fit <- extract_fit_parsnip(final_nn_fit)  
nn_model    <- tidymodels_fit$fit   
...
# Define the actual input feature names in the correct order
...
#   "Vigilance: Score 2",
#   "Vigilance: Score 3",
#   "Gaze-Head-Deviation: Score 1",
#   "Gaze-Head-Deviation: Score 2",
#   "Gaze-Head-Deviation: Score 3",
#   "Hemiparesis: Score 1",
#   "Hemiparesis: Score 2",
#   "Hemiparesis: Score 3",
#   "Aphasia/Dysarthria = No",
#   "Aphasia/Dysarthria = Yes",
#   "Medication Anticoags = No",
#   "Medication Anticoags = Yes",
#   "TOO Exclusion Criteria = No",
#   "TOO Exclusion Criteria = Yes",
#   "Pupil Reaction Abnormal = No",
#   "Pupil Reaction Abnormal = Yes"
# )

NeuralNetTools::plotnet(
  nn_model,
  x_names   = feature_names,
  var_labs   = TRUE,   # Plot features
  cex_val    = 0.5,    # Size of text
  alpha_val  = 0.1,    
  circle_cex = .1      # Size of nodes
)
...
# Compute raw Garson importances (no plot)
imp_raw <- NeuralNetTools::garson(nn_model, bar_plot = FALSE)

# 2.3 Unlist to get a plain named vector
imp_vec <- if (is.list(imp_raw)) unlist(imp_raw) else imp_raw

# Assign readable variable names
names(imp_vec) <- feature_names

# Build a tibble and sort by descending importance
library(tibble)
library(dplyr)

feature_importance <- tibble(
  feature = names(imp_vec),
  rel_imp = as.numeric(imp_vec)
) %>%
...
ggplot(feature_importance, aes(
    x = reorder(feature, rel_imp),
    y = rel_imp
  )) +
  geom_col(fill = "#E18727E6") +
  coord_flip() +
  labs(
    x     = "Feature",
    y     = "Relative Importance",
    title = "Garson Feature Importance"
  ) +
...
# NeuralNetTools::lekprofile(nn_model)
...
saveRDS(nn_hyperparameter_workflow, "models/final_nn_workflow.rds")

# Save the final model
saveRDS(final_nn_fit, "models/final_nn_model.rds")
