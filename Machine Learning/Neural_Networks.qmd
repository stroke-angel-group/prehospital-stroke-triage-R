...
set.seed(456)
bootstraps <- bootstraps(nn_predictions_test, times = 1000)
...
calc_boot_metrics <- function(split) {
  # extract the analysis portion (training data) from the split
  ...
  # “scores.class0 is the vector of scores belonging to the class of interest (events),
  #  scores.class1 the scores for the other class.”
  ...
  # compute scalar performance metrics with yardstick
  tibble::tibble(
...
**Calculate metrics for all bootstraps**

```{r}

# Calculate metrics for all bootstraps
boot_metrics <- bootstraps %>%
  mutate(metrics = map(splits, ~calc_boot_metrics(.))) %>%
  select(metrics) %>%
  unnest(metrics)

# Calculate summary statistics
metrics_and_CI <- boot_metrics %>%
  ...
  pivot_wider(names_from = stat,
              values_from = value) %>%
  mutate(CI_95 = sprintf("(%.3f - %.3f)", lower, upper)) %>%
  select(metric, value, CI_95)

print(metrics_and_CI)
