...
abstract: "This document shows the process of training kNN models for preclinical stroke triage. It covers data preprocessing steps, including handling missing values via bagged tree imputation and scaling, followed by hyperparameter tuning using Racing Methods and, alternatively, grid search. The document compares the performance of the tuned Random Forest model against a fixed hyperparameter model and the 4-Item Stroke Scale (4I-SS) on a held-out test set. Evaluation metrics, including ROC and PR curves, along with confusion matrices and confidence intervals obtained through bootstrapping, are presented to assess model efficacy."
editor: visual
---
...
fold_counts <- map(cv_folds$splits, ~{
  ...
  tibble(
    fold_id = .x$id,
    # Number of EVT cases in the training set
    train_evt_count = sum(analysis_data$EVT == "1"),  
    # Number of EVT cases in the test set
    test_evt_count = sum(assessment_data$EVT == "1")  
  )
})
...
ggplot(fold_counts_df, aes(x = test_evt_count)) +
  ...
  scale_x_continuous(
    breaks = seq(min(fold_counts_df$test_evt_count),
                 max(fold_counts_df$test_evt_count), by = 1)
  ) +
  labs(
    title = "",
...
knn_parameter_set <- parameters(
  neighbors(range = c(5L, 25L)),
  over_ratio(range = c(0.2, 0.5)),
  under_ratio(range = c(0.8, 1.2))
)

# Create a grid for Race ANOVA
knn_parameter_grid_race <- grid_space_filling(knn_parameter_set,
                                                  size = 50)
...
knn_parameter_grid_search <- grid_regular(knn_parameter_set,
...
knn_tune_results <- tune_race_anova(
  knn_workflow,
  resamples = cv_folds,                
  grid      = knn_parameter_grid_race,                
...
#     knn_workflow,                          
#     resamples = cv_folds,                
#     grid      = knn_parameter_grid_search, 
#     metrics   = metrics_tune_grid)           
...
# knn_tune_results$.metrics

```

**Show best results based on a selected peknnormance metric**
...
**Select optimal Hyperparameter-Set based on a peknnormance metrics**
...
best_knn <- select_best(knn_tune_results,
                       metric = "roc_auc")
...
all_fold_metrics <- collect_metrics(knn_tune_results, summarize = FALSE)
...
plot_hyperparam <- function(data, param, label) {
  summary_df <- data %>%
    ...
    mutate(min_estimate = pmin(min_estimate, max_estimate))

  ggplot(summary_df, aes(x = !!sym(param), y = mean_estimate, color = Metric_Label)) +
...
**SMOTE Oversampling Ratio (`over_ratio`)**
...
if ("over_ratio" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "over_ratio",
                        label = "SMOTE Oversampling Ratio (over_ratio)"))
}
...
if ("neighbors" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "neighbors",
...
knn_metrics <- tune::collect_metrics(knn_tune_results)

knn_hyperparams <- knn_metrics %>% 
  select(-.metric, -.estimator, -mean, -n, -std_err, -.config)

# Plot all available Hyperparameters
print(colnames(knn_hyperparams)) 
...
hyperparameter_1 <- "neighbors"   
hyperparameter_2 <- "under_ratio"   
hyperparameter_3 <- "over_ratio"    
...
pr_auc_subset_3_params <- knn_metrics %>%
...
knn_hyperparameter_workflow <- finalize_workflow(knn_workflow, best_knn)
knn_hyperparameter_workflow
...
final_knn_fit <- fit(knn_hyperparameter_workflow, data = train_data)
final_knn_fit
...
# final_knn_fit <- fit(knn_workflow, data = train_data)
# 
# final_knn_fit
...
test_probs <- predict(final_knn_fit, new_data = test_data, type = "prob") %>%
...
knn_predictions_train <- predict(final_knn_fit,
                                 new_data = train_data,
                                 type = "prob") %>%
  mutate(
...
knn_predictions_test <- predict(final_knn_fit,
                                new_data = test_data,
                                type = "prob") %>%
  mutate(
    pred_class = if_else(.pred_1 >= custom_threshold, "1", "0"), # Apply custom threshold
...
cm_knn_train <- confusionMatrix(data = knn_predictions_train$pred_class,
                reference = knn_predictions_train$EVT,
                positive = "1")
...
cm_knn_test <- confusionMatrix(data = knn_predictions_test$pred_class,
                reference = knn_predictions_test$EVT,
                positive = "1")
...
boot_results <- boot::boot(
  data      = knn_predictions_test,       # dataset to bootstrapp      
...
# Training
probabilities_ml_train <- knn_predictions_train$.pred_1

# Testing
probabilities_ml_test <- knn_predictions_test$.pred_1
...
conf_matrix_counts <- knn_predictions_test %>%
  mutate(
    type = case_when(
...
# Define the KNN model
knn_spec_fixed <- nearest_neighbor(
  neighbors = 25,
  mode = "classification"
) %>%
  set_engine("kknn")
...
custom_over_ratio  <- 0.3040816
custom_under_ratio <- 1.093878
...
evaluate_fold_knn <- function(split,
  ...
  fold_recipe <- recipe(
    ...
    step_mutate(across(all_nominal_predictors(), ~ as.numeric(as.character(.x)))) %>%
    step_range(all_numeric_predictors(), min = 0, max = 1) %>%
    step_downsample(EVT, under_ratio = under_ratio, skip = TRUE, seed = seed) %>% 
    step_smotenc(EVT, over_ratio = over_ratio, neighbors = 5, skip = TRUE)
  ...
  list(
    aucroc = as.numeric(pROC::auc(pROC::roc(test_outer$EVT, probs, quiet = TRUE))),
...
metrics_df_folds <- outer_results_knn %>% 
  transmute(
...
cm_list <- map(outer_results_knn$results, "cm")
...
Calculate the Confidence Intervals using 1000 Bootstrap iterations.
...
metrics_df_ci <- outer_results_knn %>%                 
  transmute(
    Fold      = id,
    AUCROC    = map_dbl(results, "aucroc"),
    AUCPR     = map_dbl(results, "aucpr"),
    Accuracy  = map_dbl(results, "acc"),
    Precision = map_dbl(results, "prec"),
    F1        = map_dbl(results, "f1"),         
    Sens      = map_dbl(results, "sens"),
    Spec      = map_dbl(results, "spec"),
    NPV       = map_dbl(results, "npv"),
    Brier     = map_dbl(results, "brier")
  )

ci_bootstrap <- map_dfr(
  1:boot_iters,
  ~ metrics_df_ci %>% 
...
# format function for the result (mean, lower bound, upper bound)
table_format_function <- \(m, l, u) sprintf("%.2f (%.2f–%.2f)", m, l, u)

outer_fold_results_CI <- ci_bootstrap %>%                       
  pivot_longer(everything(),
               names_to   = c("Metric", ".stat"),
               names_sep  = "_") %>%
  pivot_wider(names_from = .stat, values_from = value) %>%
  mutate(`Model Result (95% CI)` = table_format_function(mean, lower, upper)) %>%
  select(Metric, `Model Result (95% CI)`) %>%
  arrange(match(Metric,
                c("AUCROC","AUCPR",
                  "Accuracy",
                  "Sens","Spec","Precision",
                  "NPV","F1","Brier")))

print(outer_fold_results_CI)
...
Calculate the Confidence Intervals using 2000 Bootstrap iterations.
...
ci_bootstrap <- map_dfr(
  1:boot_iters,
  ~ metrics_df %>%
      slice_sample(n = nrow(metrics_df), replace = TRUE) %>%
      summarise(across(where(is.numeric), mean))
) %>%
  summarise(across(everything(), list(
    mean  = mean,
...
outer_fold_results_CI <- ci_bootstrap %>%                       
  pivot_longer(everything(),
               ...
               names_sep  = "_") %>%
  pivot_wider(names_from = .stat, values_from = value) %>%
  # here we use table_format_function() instead of raw sprintf()
  mutate(`4I-SS Result (95% CI)` = table_format_function(mean, lower, upper)) %>%
  select(Metric, `4I-SS Result (95% CI)`) %>%
  arrange(match(Metric,
              c("AUCROC","AUCPR",
                "Accuracy",
                "Sens","Spec","Precision",
...
knn_outer_preds  <- map_dfr(outer_results_knn$results,
...
# Random-Forest ROC
roc_knn <- pROC::roc(knn_outer_preds$truth, knn_outer_preds$score,
...
saveRDS(knn_hyperparameter_workflow, "models/final_knn_workflow.rds")

# Save the final model
saveRDS(final_knn_fit, "models/final_knn_model.rds")
