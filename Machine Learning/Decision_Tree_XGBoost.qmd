...
# XGBoost
library(xgboost) # Engines XGBoost
library(rules)
library(hardhat)
library(dials)
...
An higher value indicates **less** weight on FN (does not work with a variable and parallel processing, instead 14 must be define within `set_engine()`)
...
xgb_spec <- boost_tree(
  ...
  set_engine(
    ...
    eval_metric           = "aucpr",   # metric used for early stopping
...
xgb_parameter_set <- parameters(
  trees(range            = c(200L, 1000L)),                
  learn_rate(range       = c(0.01, 0.30), trans = log10_trans()), 
  tree_depth(range       = c(2L, 12L)),
  min_n(range            = c(1L, 4L)),
  loss_reduction(range   = c(0, 3)),
  sample_prop(range      = c(0.6, 1.0)),                  
  mtry(range             = c(5L, 12L)),                   
  scale_pos_weight(range = c(7, 25))
)

# Create a grid for Race ANOVA
xgb_parameter_grid_race <- grid_space_filling(xgb_parameter_set,
                                              size = 60
                                              )
...
xgb_parameter_grid_search <- grid_regular(xgb_parameter_set,
...
xgb_tune_results <- tune_race_anova(
  xgb_workflow,
  resamples = cv_folds,                
  grid      = xgb_parameter_grid_race,                
...
#     xgb_workflow,                          
#     resamples = cv_folds,                
#     grid      = xgb_parameter_grid_search, 
#     metrics   = metrics_tune_grid)           
...
best_xgb <- select_best(xgb_tune_results,
                       metric = "roc_auc")
...
all_fold_metrics <- collect_metrics(xgb_tune_results, summarize = FALSE)
...
if ("learn_rate" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "learn_rate",
                        label = "Learning Rate"))
}
...
if ("loss_reduction" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
...
if ("sample_size" %in% names(prepared_data)) {
  print(plot_hyperparam(prepared_data,
                        param = "sample_size",
                        label = "Sample Size"))
}
...
xgb_metrics <- tune::collect_metrics(xgb_tune_results)

xgb_hyperparams <- xgb_metrics %>% 
  select(-.metric, -.estimator, -mean, -n, -std_err, -.config)

# Plot all available Hyperparameters
print(colnames(xgb_hyperparams)) 
...
hyperparameter_1 <- "tree_depth"   
hyperparameter_2 <- "scale_pos_weight"   
...
pr_auc_subset_3_params <- xgb_metrics %>%
...
xgb_hyperparameter_workflow <- finalize_workflow(xgb_workflow, best_xgb)
xgb_hyperparameter_workflow
...
final_xgb_fit <- fit(xgb_hyperparameter_workflow, data = train_data)
final_xgb_fit
...
# final_xgb_fit <- fit(xgb_workflow, data = train_data)
# 
# final_xgb_fit
...
test_probs <- predict(final_xgb_fit, new_data = test_data, type = "prob") %>%
...
xgb_predictions_train <- predict(final_xgb_fit,
                                 new_data = train_data,
                                 type = "prob") %>%
  mutate(
...
train_probs <- predict(final_xgb_fit, new_data = train_data, type = "prob") %>%
...
# Define a custom threshold
custom_threshold <- 0.4  # The lower the threshold, the more likely the model will predict a positive class ("1")

# Make predictions with probabilities for the test data
xgb_predictions_test <- predict(final_xgb_fit,
                                new_data = test_data,
                                type = "prob") %>%
  mutate(
    pred_class = if_else(.pred_1 >= custom_threshold, "1", "0"), # Apply custom threshold
...
cm_xgb_train <- confusionMatrix(data = xgb_predictions_train$pred_class,
                reference = xgb_predictions_train$EVT,
                positive = "1")
...
cm_xgb_test <- confusionMatrix(data = xgb_predictions_test$pred_class,
                reference = xgb_predictions_test$EVT,
                positive = "1")
...
boot_results <- boot::boot(
  data      = xgb_predictions_test,       # dataset to bootstrapp      
...
# Training
probabilities_ml_train <- xgb_predictions_train$.pred_1

# Testing
probabilities_ml_test <- xgb_predictions_test$.pred_1
...
conf_matrix_counts <- xgb_predictions_test %>%
  mutate(
    type = case_when(
...
This chunk executes a **5‑fold *outer* cross‑validation** on a fixed XGB specification. We skip the inner tuning loop because:
...
Hyperparameters from section above: `select_best(xgb_tune_results, metric = "roc_auc")`
...
xgb_spec_fixed <- boost_tree(
  ...
  mtry          = 9,
  learn_rate    = 1.023293,
  loss_reduction= 85.54673,
  sample_size   = 0.8915254
) %>%
  set_engine("xgboost", 
             scale_pos_weight      = 11.58,
             eval_metric           = "aucpr",
             early_stopping_rounds = 30)
...
evaluate_fold_xgb <- function(split,
  ...
  fold_recipe <- recipe(
    ...
    step_mutate_at(all_nominal_predictors(),
                   fn = ~ as.numeric(as.character(.x))) 

  # Fit 
  fit_outer <- workflow() %>% 
    add_recipe(fold_recipe) %>% 
    add_model(xgb_spec_fixed) %>% 
    fit(train_outer)
...
metrics_df_folds <- outer_results_xgb %>% 
  transmute(
...
cm_list <- map(outer_results_xgb$results, "cm")
...
xgb_outer_preds  <- map_dfr(outer_results_xgb$results,
...
# Random-Forest ROC
roc_xgb <- pROC::roc(xgb_outer_preds$truth, xgb_outer_preds$score,
...
imp_dt <- xgb.importance(
  model = xgb_model
)

importance_table <- imp_dt %>%
  as_tibble() %>%                 
  select(Feature, Gain) %>%      
...
saveRDS(xgb_hyperparameter_workflow, "models/final_xgb_workflow.rds")

# Save the final model
saveRDS(final_xgb_fit, "models/final_xgb_model.rds")
